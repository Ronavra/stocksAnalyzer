...
def generate(db, top=5, horizons=(5,10,20)):
...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument("--top",type=int,default=5); ap.add_argument("--horizons",nargs="+",type=int,choices=[5,10,20],default=[5,10,20]); a=ap.parse_args()
    picks=generate(get_supabase(),a.top,tuple(a.horizons))
    print(f"Saved {len(picks)} frozen UP candidates across horizons {a.horizons}")
